 ...
 *
 */
public class EndItinerary extends TeqObjects
{	
	...
	private EndItinerary()
	{
		super();
	}
	
	public static EndItinerary getFromCSV(String csv, long notification, String ack_timestamp, 
			String notification_timestamp, String notification_valid_timestamp) throws Exception
	{
		String[] list = csv.split(",(?=([^\"]*\"[^\"]*\")*[^\"]*$)");
		EndItinerary myVehicleTracking = null;
		if(list.length == 12)
		{
			myVehicleTracking = new EndItinerary();
	...
	{
		return "EndItinerary: " + this.getVehicleCode() + ";" + this.getTripCode();
	}
